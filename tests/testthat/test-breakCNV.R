...
test_that(".createITVData returns expected results", {
  ...
  expect_equal(.createITVData(cnv), expected)
})
...
test_that("`Break_CNV_interval()` returns expected results; test 1", {
  ...
  cnv <- data.frame("ID" = 1L:10L,
                    ...
                    "TYPE" = c(0L, 0L, 1L, 1L,
                               0L, 3L, 3L, 3L,
                               4L, 4L))
  expected <- list("CNV_frag_l" = data.frame("ID" = 1L:10L,
                                             "CHR" = rep(22L, 10L),
                                             "idx" = seq(1L, 20L, 2L),
                                             "TYPE" = c(0L, 0L, 1L, 1L,
                                                        0L, 3L, 3L, 3L,
                                             ...
                                             "AUC" = c(2.0, 2.0, 1.0, 1.0,
                                                       2.0, 1.0, 1.0, 1.0,
                                                       2.0, 2.0)),
                   "ITV_info" = data.frame("CHR" = rep(22L, 19L),
                                           "idx" = 1L:19L,
                                           "ITV1" = 1L:19L,
                                           "ITV2" = 2L:20L))
...
test_that("`Break_CNV_interval()` returns expected results; test 2", {
  ...
  expected <- list("CNV_frag_l" = data.frame("ID" = c(1L, 2L, 2L, 3L, 3L, 4L),
                                             "CHR" = rep(22L, 6L),
                                             "idx" = c(1L, 3L, 4L, 4L, 5L, 7L),
                                             "TYPE" = c(0L, 0L, 0L, 1L, 1L, 1L),
                                             "deldup" = c(rep("del", 6)),
                                             "AUC" = c(2*4, 2*1, 2*1, 1*1, 1*3, 1*15)),
                   "ITV_info" = data.frame("CHR" = rep(22L, 7L),
                                           "idx" = 1L:7L,
                                           "ITV1" = c(1L, 5L, 10L, 11L, 12L, 15L, 20L),
                                           "ITV2" = c(5L, 10L, 11L, 12L, 15L, 20L, 35L)))
  expect_equal(.breakCNV(cnv), expected)
})
...
test_that("`Break_CNV_interval()` returns expected results; test 3", {
  ...
  alternative_func <- function(cnv) {
    ...
    ITV_info <- data.frame("CHR" = rep(cnv$CHR[1L], length(BPs) - 1L),
                           "idx" = 1L:{length(BPs) - 1L},
                           "ITV1" = utils::head(BPs, -1L),
                           "ITV2" = utils::tail(BPs, -1L))
    
    
    list("CNV_frag_l" = long_form[order(long_form$idx), ],
         "ITV_info" = ITV_info)
  }
