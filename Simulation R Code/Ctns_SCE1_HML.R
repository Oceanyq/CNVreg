## Code used on HPC cluster to simulate continuous scenario 1 with 
## causal signals in the middle of a consecutive CNV region
vi ctns_wt_CV.R
...
#args =c('pw_1', -16, 16, 2, -4, -6, -0.5, 10, 0.6, "6")
...
# output files
in_label=c("05")
out_label=c(paste0("~/out",b_tag1,"/ctns",b_tag1),b_tag1)
...
for (b_tag in c("1", "3", "6")){
 ...
 for (wt in c("pw_1","pw_cs")){      
    if(!file.exists(paste0("ctns_", wt, b_tag, ".bash"))){
        file.create(paste0("ctns_",wt,b_tag, ".bash"))
    }
          
        fileConn<-file(paste0("ctns_",wt,b_tag, ".bash"))

        writeLines(c("#!/bin/bash",
                     paste0("#SBATCH --output=S1Prtl_ctns", wt, b_tag, ".out"),
                    "hostname",
                    "pwd",
        #fit weighted smoothness
        paste0("/opt/R/4.1.3/bin/Rscript ctns_wt_CV.R ",wt," ",V2_1," ", V2_2, " ", V2_p," ", V1_1, " ",V1_2, " ", V1_p, " ", T_rep," ",b_1," ",b_tag)
        ), fileConn)
...
for (b_tag in c("1", "3","6")){
    for (wt in c("pw_1","pw_cs")){
     ...
     print(paste0("sbatch ctns_",wt, b_tag,".bash"))
    }
}
