 ...
 for (wt in c("pw_1","pw_cs")){      
    if(!file.exists(paste0("ctns_", wt, b_tag, ".bash"))){
        ...
        writeLines(c("#!/bin/bash",
                     paste0("#SBATCH --output=S2TX_ctns", wt, b_tag, ".out"),
                    "hostname",
                    "pwd",
        #fit weighted smoothness
        paste0("/opt/R/4.1.3/bin/Rscript S2TX_ctns_wt_CV.R ",wt," ",V2_1," ", V2_2, " ", V2_p," ", V1_1, " ",V1_2, " ", V1_p, " ", T_rep," ",b_1," ",b_tag)
        ), fileConn)
