## generate Simulation data on HPC with causal CNVs on 2 disconnected regions 
## over a consective CNV region.  

vi bnry_wt_s3PP.R

setwd("~/trim_05")
rm(list=ls())
##accept arguments for simulation 
...
#the predicted value are all equal no need to update
            ...
            #sqrt_v[1:10]
            # tried but failed: update X_augmented -- intercept added after transformation

            Xp1=cbind(1, X)
            colnames(Xp1)<-c(paste0("X",c(0:(dim(Xp1)[2]-1))))
            Ap0<-cbind(0,A_matrix)
            colnames(Ap0)<-c(paste0("X",c(0:(dim(Ap0)[2]-1))))
            #X_aug<-rbind(Xp1,sqrt_term*Ap0)
