...
#'
cvfit_WTSMTH <- function(data, lambda1= seq(-8, 0, 1), lambda2= seq(-8, 8, 1), weight,
  ...
  loss_list <- foreach::foreach(i = seq_len(nrow(idx)), 
                                #.packages = c("CNVreg"),
                               #.export = c("fit_WTSMTH", ".loss", ".isNumericVector", ".isNamedList", ".testIterControl", ".confirmContinuous", ".confirmBinary", ".isIntegerVector"),
                                .combine = "rbind") %dopar% {
                                  
                                  
                                  if(verbose){
                                    formatted_string <- sprintf("Values: %8.1f %8.1f %8.1f",  idx[i, 1L], lambda1[idx[i, 2L]], lambda2[idx[i, 3L]])
                                  ...
                                  beta_lmd21 <- fit_WTSMTH(data = data, 
                                                           ...
                                                           subset = subset)
                                  #data = data; lambda1 = -5; lambda2 = 9; family = family; iter.control = iter.control;subset = subset
                                  #evaluate loss
                                  ...
                                  loss <- .loss(X = data$XZ[!subset, ], 
                                                ...
                                                family = family)
                                  #print(c(lambda1[idx[i, 2L]], lambda2[idx[i, 3L]], loss))
...
# X = data$XZ[!subset, ];  Y = data$Y[!subset]; beta = beta_lmd21$coef;  family = family
 ...
 # loss_matrix |> print()
  ...
  colnames(loss) <- c("Lambda2",colnames(loss)[-1])
  #print(loss)
  #print(c("selected.lambda",b_lmd1, b_lmd2))
  list("Loss" = loss,
